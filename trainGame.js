/* ========================================
   trainGame.js — 安抚音符（踏切犬）
   ======================================== */

const TrainGameScene = {
    canvas: null,
    ctx: null,
    ...
    // Tool (Poem Page from BookSpirit)
    tool: {
        x: 0, y: 0,
        width: 50,
        durability: 2,
    ...
    init(canvas, ctx) {
        this.canvas = canvas;
        this.ctx = ctx;
        this.config = YokaiData.trainDog.gameConfig;
    ...
    draw() {
        ...
        for (const drop of this.drops) {
            if (drop.type === 'rain') {
                ctx.fillStyle = 'rgba(255, 71, 87, 0.6)'; // Red noise/sparks
                ctx.fillRect(Math.floor(drop.x), Math.floor(drop.y), 2, 8);
                ctx.shadowColor = '#ff4757';
                ctx.shadowBlur = 10;
                ctx.fillStyle = '#ffffff';
                // Draw a musical note shape
                ctx.beginPath();
                ctx.arc(drop.x - 3, drop.y + 5, 4, 0, Math.PI * 2);
                ctx.fill();
                ctx.fillRect(drop.x + 1, drop.y - 5, 2, 10);
                ctx.fillRect(drop.x + 1, drop.y - 5, 6, 2);
                return;
                ctx.fill();
                ctx.beginPath();
        ...
        if (this.comboCount >= 5 && this.comboTimer > 0) {
            ...
            Pixel.drawText(ctx, `守护心弦！回归诗意的平静。`, w / 2, h * 0.8, {
                size: 18,
                color: '#ff4757',
                align: 'center'
            });
            Pixel.drawText(ctx, `(手中紧握：书灵的忘却之页)`, w / 2, h * 0.84, {
                size: 12,
        ...
        ctx.fillStyle = progress > 0.8 ? '#2ed573' : '#ff4757';
    ...
    _drawTool(ctx) {
        ...
        for(let i=0; i<5; i++) {
            ctx.beginPath();
            ctx.moveTo(cx - 15, cy - 15 + i*8);
            ctx.lineTo(cx + 15, cy - 15 + i*8);
            ctx.stroke();
        }
