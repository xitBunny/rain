...
const RainGameScene = {
    ...
    init(canvas, ctx) {
        this.canvas = canvas;
        this.ctx = ctx;
        this.config = YokaiData.rainWoman.gameConfig;
    ...
    draw() {
        ...
        for (const drop of this.drops) {
            if (drop.type === 'rain') {
                ctx.fillStyle = 'rgba(110, 198, 255, 0.7)';
                ctx.fillRect(Math.floor(drop.x), Math.floor(drop.y), 2, 6);
            } else if (drop.type === 'tear') {
                // Tear drop — blue, slightly larger, glowing
                ctx.save();
        ...
        if (this.comboCount >= 5 && this.comboTimer > 0) {
            ...
            Pixel.drawText(ctx, comboText, w / 2, h * 0.3, {
    ...
    _drawUmbrella(ctx) {
        ...
        // Flash on hit
        if (u.hitFlash > 0) {
            ctx.globalAlpha = 0.5 + u.hitFlash;
        }
        ...
        if (u.broken) {
            ...
            ctx.lineTo(cx - halfW / 2, cy - 8);
            ctx.lineTo(cx, cy - 3);
            ctx.lineTo(cx + halfW / 2, cy - 10);
            ...
            umbrellGrad.addColorStop(0, '#d4587a');
            umbrellGrad.addColorStop(0.5, '#e8834a');
            umbrellGrad.addColorStop(1, '#d4587a');
            ...
            ctx.lineWidth = 1;
            for (let i = 1; i < 4; i++) {
                const sx = cx - halfW + (u.width / 4) * i;
                ctx.beginPath();
                ctx.moveTo(sx, cy);
                ctx.lineTo(sx, cy - Math.sqrt(halfW * halfW - (sx - cx) * (sx - cx)));
                ctx.stroke();
            }
            ...
            for (let i = 0; i < segments; i++) {
                const angle1 = Math.PI + (Math.PI / segments) * i;
                const angle2 = Math.PI + (Math.PI / segments) * (i + 1);
                const x1 = cx + Math.cos(angle1) * halfW;
                const x2 = cx + Math.cos(angle2) * halfW;
                const mx = (x1 + x2) / 2;
                ctx.beginPath();
                ctx.arc(mx, cy, (x2 - x1) / 2, 0, Math.PI);
                ctx.fill();
            }
            ...
            ctx.beginPath();
            ctx.arc(cx, cy - halfW, 2, 0, Math.PI * 2);
            ctx.fill();
            ...
            if (Math.random() < 0.15) {
                ...
                this.splashParticles.emit({
                    ...
                    y: cy + 2,
                    count: 1,
                    speedX: () => side * (0.5 + Math.random()),
                    speedY: () => 0.5 + Math.random(),
                    life: 20,
                    size: 1,
                    color: 'rgba(110, 198, 255, 0.5)',
                    gravity: 0.15,
                    fadeOut: true
                });
            }
